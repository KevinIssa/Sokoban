...
bool Sokoban::can_tp(){
    ...
    for(int i=0 ; i<teleporter_cell.size(); i++ ){
        ...
        if (level_cell[id(teleporter_cell[i].x, teleporter_cell[i].y)].get_repr() == '@'){
            int other_tp = (i-1)%2;
            if(level_cell[id(teleporter_cell[other_tp].x , teleporter_cell[other_tp].y)].get_repr() !=  '&')
                possible = false;
        }
    }
